...
class Dataset(torch.utils.data.Dataset):
  def __init__(self, df):
      ...
      self.texts = [tokenizer(str(text), padding='max_length', max_length = 512, truncation=True, return_tensors="pt") for text in df['article']]
...
def test_model(model, test_data, batch_size=2):
  ...
  with torch.no_grad():
    for test_input, test_label in tqdm(test_dataloader):
        ...
        train_label = test_label.to(device)
  ...
  print("Test Accuracy of", total_acc_test)
