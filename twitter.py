...
import csv
import re
from preprocess import remove_newlines_vec
...
def get_senator_usernames():
    df = pd.read_csv(
        "/Users/vruthikthakkar/Desktop/487-final-project/data/govtrack-stats-2020-senate-ideology.csv")
...
def get_user_tweets(username):
    ...
    #     print(tweets.data)
    tweets = client.get_users_tweets(id=get_user_id(username), tweet_fields=[
                                     'context_annotations', 'created_at', 'geo'], max_results=100, exclude=['retweets', 'replies'])
    tweets_text = []
...
if __name__ == '__main__':
    ...
    with open('/Users/vruthikthakkar/Desktop/487-final-project/data/tweets_2.csv', 'w') as f:
        writer = csv.writer(f)
        ...
        for row in senators.iterrows():
            ...
            for tweet in text:
                ...
                writer.writerow(
                    [row_items['name'], row_items['username'], row_items['label'], tweet])

    df = pd.read_csv(
        '/Users/vruthikthakkar/Desktop/487-final-project/data/tweets_2.csv')
    df['tweet'] = remove_newlines_vec(df['tweet'])
    df.to_csv(
        '/Users/vruthikthakkar/Desktop/487-final-project/data/tweets_2.csv', index=False)
